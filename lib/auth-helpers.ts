 ...
 */
export async function ensureStudentProfile(accessToken: string): Promise<{ ok: boolean; error?: string; hasOnboarded?: boolean }> {
  try {
    ...
    const data = (await res.json().catch(() => ({}))) as { error?: string; hasOnboarded?: boolean };
    if (!res.ok) {
      console.error("[ensureStudentProfile]", res.status, data.error);
      return { ok: false, error: data.error ?? `HTTP ${res.status}` };
    }
    return { ok: true, hasOnboarded: data.hasOnboarded };
  } catch (e) {
